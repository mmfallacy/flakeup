...
- [x] **Command Line Interface:** `flakeup` provides a robust CLI for managing templates.
  ...
  - [x] `flakeup s[how]`: Shows the list of flakeup templates.
- [x] **Reads Custom Flake Outputs:** Reads the `flakeup` custom flake outputs.
...
- [x] `flakeup.defaultFlags.<subcommand>`: Default flags for each subcommand.
